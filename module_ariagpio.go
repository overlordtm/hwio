...
type AriaGPIOModule struct {
	name        string
	definedPins AriaGPIOModulePinDefMap
	openPins    map[Pin]*AriaGPIOModuleOpenPin
}

// Represents the definition of a GPIO pin, which should contain all the info required to open, close, read and write the pin
// using FS drivers.
type AriaGPIOModulePinDef struct {
	pin         Pin
	gpioLogical int
}

// A map of GPIO pin definitions.
type AriaGPIOModulePinDefMap map[Pin]*AriaGPIOModulePinDef

type AriaGPIOModuleOpenPin struct {
	pin          Pin
	gpioLogical  int
	gpioBaseName string
	valueFile    *os.File
...
func NewAriaGPIOModule(name string) (result *AriaGPIOModule) {
	result = &AriaGPIOModule{name: name}
	result.openPins = make(map[Pin]*AriaGPIOModuleOpenPin)
	return result
}

// Set options of the module. Parameters we look for include:
// - "pins" - an object of type AriaGPIOModulePinDefMap
func (module *AriaGPIOModule) SetOptions(options map[string]interface{}) error {
	v := options["pins"]
	if v == nil {
		return fmt.Errorf("Module '%s' SetOptions() did not get 'pins' values", module.GetName())
	}

	module.definedPins = v.(AriaGPIOModulePinDefMap)
	return nil
}

// enable GPIO module. It doesn't allocate any pins immediately.
func (module *AriaGPIOModule) Enable() error {
	return nil
}

// disables module and release any pins assigned.
func (module *AriaGPIOModule) Disable() error {
...
func (module *AriaGPIOModule) GetName() string {
	return module.name
}

func (module *AriaGPIOModule) PinMode(pin Pin, mode PinIOMode) error {
...
func (module *AriaGPIOModule) DigitalWrite(pin Pin, value int) (e error) {
...
func (module *AriaGPIOModule) DigitalRead(pin Pin) (value int, e error) {
	openPin := module.openPins[pin]
	if openPin == nil {
...
func (module *AriaGPIOModule) ClosePin(pin Pin) error {
	openPin := module.openPins[pin]
	if openPin == nil {
...
func (module *AriaGPIOModule) makeOpenGPIOPin(pin Pin) (*AriaGPIOModuleOpenPin, error) {
	p := module.definedPins[pin]
	if p == nil {
		return nil, fmt.Errorf("Pin %d is not known to GPIO module", pin)
	}

	result := &AriaGPIOModuleOpenPin{pin: pin, gpioLogical: p.gpioLogical}
...
func (op *AriaGPIOModuleOpenPin) gpioExport() error {
	bn := "/sys/class/gpio/pioA" + strconv.Itoa(op.gpioLogical)
	if !fileExists(bn) {
...
// Needs to be called to allocate the GPIO pin
func (op *AriaGPIOModuleOpenPin) gpioUnexport() error {
	s := strconv.FormatInt(int64(op.gpioLogical), 10)
...
func (op *AriaGPIOModuleOpenPin) gpioDirection(dir string) error {
	if dir != "in" && dir != "out" {
...
func (op *AriaGPIOModuleOpenPin) gpioGetValue() (int, error) {
	var b []byte
	b = make([]byte, 1)
...
func (op *AriaGPIOModuleOpenPin) gpioSetValue(value int) error {
	if op.valueFile == nil {
